  ...
  protected case class SymbolsAnalysis()(using override val symbols: Symbols) extends EffectsAnalysis {
    ...
    // declared as parameters.
     def makeFunctionTypeExplicit(tpe: Type): Type =
      def makeSigmaType(mutableParams: Seq[Type], tpe: Type, returnType: Type) =
        if mutableParams.isEmpty then tpe
         else {
           val freshParams = mutableParams.map(tpe => ValDef(FreshIdentifier("x"), tpe))
           SigmaType(freshParams, returnType).copiedFrom(returnType)
         }

      tpe match {
       case ft @ FunctionType(from, to) =>
         ft.copy(to = makeSigmaType(from.filter(isMutableType(_)), ft, to)).copiedFrom(ft)
         
       case pt @ PiType(params, to) =>
         val mutableParamTypes = params.map(_.tpe).filter(isMutableType(_))
         pt.copy(to = makeSigmaType(mutableParamTypes, pt, to)).copiedFrom(pt)
     }
    ...
    def updateFunction(fd: FunAbstraction, env: Environment): FunAbstraction = {
      ...
      if (aliasedParams.isEmpty) {
        ...
        val newBody = body.map { body =>
          ...
          val finalBody: Expr = Tuple(freshLocals.map(_.toVariable) ++ Seq(explicitBody)).copiedFrom(body)
