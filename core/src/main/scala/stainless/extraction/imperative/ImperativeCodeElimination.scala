    ...
    def toFunction(expr: Expr)(using state: State): (Expr, Expr => Expr, Map[Variable, Variable]) = {
      ...
      val (res, scope, fun): (Expr, Expr => Expr, Map[Variable, Variable]) = (expr match {
        ...
        case ite @ IfExpr(cond, tExpr, eExpr) =>
          ...
          val iteType = SigmaType(freshResultVars.map(_.toVal), newResultType)
        ...
        case m @ MatchExpr(scrut, cses) =>
          ...
          val freshResultVars = modifiedVars.map(_.freshen)
          val newResultType = typeOps.replaceFromSymbols(modifiedVars.zip(freshResultVars).toMap, res.tpe)
          val matchType = SigmaType(freshResultVars.map(_.toVal), newResultType)
            ...
            scrutScope(
              Let(tupleVd, matchE,
                Let(res,
                  ...
                  tupleSelect(tupleVd.toVariable, freshVars.length + 1, freshVars.nonEmpty)
                )
              )
            )
          }
        ...
        case alr @ ApplyLetRec(id, tparams, tpe, tps, args) if localsMapping contains id =>
          ...
          val newReturnType = SigmaType(freshResultVars.map(_.toVal), newToType)
          ...
          body match {
            case Some(bd) =>
              ...
              if (modifiedVars.isEmpty) fdWithoutSideEffects else {
                ...
                val newReturnType = SigmaType(freshResultVars.map(_.toVal), newBasicType)
    ...
    def transformPureExpr(expr: Expr)(using state: State): Expr = {
      ...
      exprOps.postMap {
        case alr @ ApplyLetRec(id, tparams, tpe, tps, args) if localsMapping contains id =>
          ...
          val newReturnType = SigmaType(freshVars.map(_.toVal), newBasicType)
